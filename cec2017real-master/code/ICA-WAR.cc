...
public:
    ...
    void removeColony(Country* colony) {
        auto it = find(colonies.begin(), colonies.end(), colony);
        if (it != colonies.end()) {
            colonies.erase(it);
        }
    }
...
public:
    ...
    void Assimilation(){
        ...
        for (int i = 0; i < Imperialists.size(); i++){
            ...
            //cout << "Colonies size: " << colonies.size() << endl;
    ...
    void War() {
    uniform_real_distribution<> dis(0.0, 1.0); // Distribución para la probabilidad de guerra

    for (int i = 0; i < Imperialists.size(); i++) {
        for (int j = i + 1; j < Imperialists.size(); j++) {
            Imperialist* imp1 = Imperialists[i];
            Imperialist* imp2 = Imperialists[j];

            // Calcular la distancia euclídea entre los dos imperialistas
            double distance = EuclideanDistance(*imp1, *imp2);

            // La probabilidad de guerra es mayor cuanto más cerca estén los imperialistas
            double warProbability = (1 - distance / sqrt(10 * pow(200, 2))) /2;
            if (warProbability < 0) warProbability = 0;
            //cout << "War probability: " << warProbability << endl;

            // Si se cumple la condición de guerra, los imperialistas entran en guerra
            if (dis(gen) < warProbability) {
                // Barajar las colonias de los dos imperios
                vector<Country*> colonies1 = imp1->getColonies();
                vector<Country*> colonies2 = imp2->getColonies();
                random_shuffle(colonies1.begin(), colonies1.end());
                random_shuffle(colonies2.begin(), colonies2.end());

                // Competir cada colonia 1 a 1
                for (int k = 0; k < min(colonies1.size(), colonies2.size())-1; k++) {
                    Country* col1 = colonies1[k];
                    Country* col2 = colonies2[k];

                    // El imperio que gane el enfrentamiento se queda con la colonia rival
                    if (col1->getFitness() < col2->getFitness()) {
                        // El imperio 1 gana y se queda con la colonia 2
                        imp1->addColony(col2);
                        imp2->removeColony(col2);
                    } else {
                        // El imperio 2 gana y se queda con la colonia 1
                        imp2->addColony(col1);
                        imp1->removeColony(col1);
                    }
                }
            }
        }
