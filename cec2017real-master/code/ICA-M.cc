...
public:
    ...
    //cout << "Eval " << evals << " Empires: " << Imperialists.size() << endl;

    //Código para depuración (opcional)
    // for (int i = 0; i < Imperialists.size(); i++){
    //     vector<Country*> colonies = Imperialists[i]->getColonies();
    //     cout << "Imperialist " << i << " Fitness: " << Imperialists[i]->getFitness() << endl;
    //     for (int j = 0; j < colonies.size(); j++){
    //         cout << "Imperialist " << i << " Colony " << j << " Fitness: " << colonies[j]->getFitness() << endl;
    //     }
    //     cout << endl << endl;
    // }
}
    ...
    void Assimilation(){
        ...
        uniform_real_distribution<> dis3(0.0, 1.0); // Distribución para la probabilidad de que el imperialista se mueva
    ...
    for (int i = 0; i < Imperialists.size(); i++){
        ...
        for (int j = 0; j < colonies.size(); j++){
            ...
            // Con cierta probabilidad, permitir que el imperialista se mueva hacia la colonia
            if (dis3(gen) < 0.01) {
                vector<double> imp_sol = imp->getSolution();
                advance(imp_sol, col->getSolution(), 3);
                double imp_fitness = cec17_fitness(&imp_sol[0]);
                imp->setSolution(imp_sol);
                imp->setFitness(imp_fitness);
                evals++;
            }
        }
    }
    ...
    // Descomentar las siguientes líneas para depuración y verificación
    /*
    std::cout << "After Revolution" << std::endl;
    for (int i = 0; i < Imperialists.size(); i++){
        vector<Country*> colonies = Imperialists[i]->getColonies();
        std::cout << "Imperialist " << i << " Fitness: " << Imperialists[i]->getFitness() << std::endl;
        for (int j = 0; j < colonies.size(); j++){
            std::cout << "Imperialist " << i << " Colony " << j << " Fitness: " << colonies[j]->getFitness() << std::endl;
        }
    }
    */
}
...
int main() {
  ...
  for (int funcid = 1; funcid <= 30; funcid++) {
    ...
    ICA ica(dim, 10000*dim, 0.5, CountryList, 3.1416/3, 10, 0.3, 0.0, seed);
