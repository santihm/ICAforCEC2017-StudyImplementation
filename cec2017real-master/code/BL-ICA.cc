...
void LocalSearch(int evaluaciones) {
    double fitness = getFitness();
    int evals = 0;

    while (evals < evaluaciones) {
        double bestFitness = fitness;
        std::vector<double> bestSolution = solution;

        // Generar todas las soluciones vecinas posibles
        for (int j = 0; j < solution.size() && evals < evaluaciones; j++) {
            std::vector<double> newSolution = solution;
            newSolution[j] += (rand() / (double)RAND_MAX) * 2 - 1;

            // Check if the solution is within the valid range
            if (newSolution[j] < -100) {
                newSolution[j] = -100;
            } else if (newSolution[j] > 100) {
                newSolution[j] = 100;
            }

            // Calcular la nueva aptitud
            double newFitness = cec17_fitness(&newSolution[0]);
            evals++;

            if (newFitness < bestFitness) {
                // Si la nueva aptitud es mejor, actualizar la mejor aptitud y la mejor solución
                bestFitness = newFitness;
                bestSolution = newSolution;
            }
        }

        // Actualizar la solución y la aptitud con la mejor solución encontrada
        fitness = bestFitness;
        solution = bestSolution;
    }

    // Actualizar la solución y la aptitud
...
public:
    ...
    void Competition(vector<double> &sol, double &fitness){
        ...
        while (evals<max_evals){
            ...
            EmpireElimination();
            if (dis(gen) < 0.15){
                for (int i=0; i<Imperialists.size() && evals+dim<max_evals; i++){
                    Imperialists[i]->LocalSearch(dim);
                    evals += dim;
                }
            }
        }
...
int main() {
  ...
  for (int funcid = 1; funcid <= 30; funcid++) {
    ...
    ICA ica(dim, 10000*dim, 0.3, CountryList, 3.1416/3, 10, 0.3, 0.05, seed);
